...
if gp.OBSERVATION_VERSION <= 7:
    blank_trans_aps = Transition(0, torch.zeros([int(np.ceil(gp.ACCESS_POINTS_FIELD / gp.SQUARE_STEP) * gp.OBSERVATION_DIMS * gp.NUM_OF_UAV),
                                                 int(np.ceil(gp.ACCESS_POINTS_FIELD / gp.SQUARE_STEP))], dtype=torch.float32), None, 0, False)
if gp.OBSERVATION_VERSION == 8:
    blank_trans_aps = Transition(0, torch.zeros([gp.OBSERVATION_DIMS * gp.NUM_OF_UAV, int(np.ceil(gp.ACCESS_POINTS_FIELD / gp.SQUARE_STEP)),
                                                 int(np.ceil(gp.ACCESS_POINTS_FIELD / gp.SQUARE_STEP))], dtype=torch.float32), None, 0, False)
if gp.GOP >= 2:
    blank_trans_sche = Transition(0, torch.zeros([gp.UAV_FIELD_OF_VIEW[0] * 2 * gp.ACCESS_POINT_PER_EDGE,
                                                  gp.UAV_FIELD_OF_VIEW[1] * gp.ACCESS_POINT_PER_EDGE * gp.NUM_OF_UAV],
                                            dtype=torch.float32), None, 0, False)
else:
    blank_trans_sche = Transition(0, torch.zeros(
        [gp.UAV_FIELD_OF_VIEW[0] * gp.ACCESS_POINT_PER_EDGE,
         gp.UAV_FIELD_OF_VIEW[1] * gp.ACCESS_POINT_PER_EDGE * gp.NUM_OF_UAV],
        dtype=torch.float32), None, 0, False)
...
class ReplayMemory:
    def __init__(self, args, capacity, typeof_black: bool, remove_function=None):
        ...
        self.memory_capacity_accesspoint = args.memory_capacity_accesspoint
        if typeof_black:
            self.history = args.history_length_accesspoint
            self.n = args.multi_step_accesspoint
        else:
            self.history = args.history_length_scheduler
    ...
    def _get_transition(self, idx):
        ...
        for t in range(self.history - 2, -1, -1):  # e.g. 2 1 0
            if transition[t + 1].timestep == 0:
                transition[t] = blank_trans_aps if self.typeof_black else blank_trans_sche
    ...
    def __next__(self):
        if self.current_idx == self.capacity:
            raise StopIteration
        if self.history <= 1:
            state = self.transitions.data[self.current_idx].state.unsqueeze(0)
            self.current_idx += 1
            return state
        ...
        for t in reversed(range(self.history - 1)):
            if prev_timestep == 0:
                state_stack[t] = blank_trans_aps.state if self.typeof_black else blank_trans_sche.state
                # If future frame has timestep 0
            else:
