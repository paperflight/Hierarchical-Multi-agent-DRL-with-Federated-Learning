...
class DQN(nn.Module):
    def __init__(self, args, action_space):
        ...
        if 'canonical' in args.architecture and '2x2' in args.architecture:
            ...
            if '2uav' in args.architecture:
                self.conv_output_size = 4096  # 3x3 2: 3328  4: 7936
            elif '4uav' in args.architecture:
            ...
            if '2uav' in args.architecture:
                self.conv_output_size = 3328  # 3x3 2: 3328  4: 7936
            elif '4uav' in args.architecture:
                self.conv_output_size = 7936
        ...
        # self.fc_h_v = nn.Sequential(nn.Linear(self.conv_output_size, 2 * args.hidden_size),
        #                        NoisyLinear(2 * args.hidden_size, args.hidden_size, std_init=args.noisy_std))
        # self.fc_h_a = nn.Sequential(nn.Linear(self.conv_output_size, 2 * args.hidden_size),
        #                        NoisyLinear(2 * args.hidden_size, args.hidden_size, std_init=args.noisy_std))
        # self.fc_z_v = NoisyLinear(args.hidden_size, self.atoms, std_init=args.noisy_std)
        # self.fc_z_a = NoisyLinear(args.hidden_size, action_space * self.atoms, std_init=args.noisy_std)

        # self.pre_h_v = nn.Linear(self.conv_output_size, 2 * args.hidden_size)
        # self.pre_z_v = nn.Linear(self.conv_output_size, 2 * args.hidden_size)
        #
        # self.fc_h_v = NoisyLinear(2 * args.hidden_size, args.hidden_size, std_init=args.noisy_std)
        # self.fc_h_a = NoisyLinear(2 * args.hidden_size, args.hidden_size, std_init=args.noisy_std)
        # self.fc_z_v = NoisyLinear(args.hidden_size, self.atoms, std_init=args.noisy_std)
        # self.fc_z_a = NoisyLinear(args.hidden_size, action_space * self.atoms, std_init=args.noisy_std)
        self.fc = nn.Sequential(nn.Linear(self.conv_output_size, args.dense_of_uav * args.hidden_size), nn.Dropout(0.2), nn.LeakyReLU())
        ...
        self.back_processor = []
        for _ in range(args.dense_of_uav):
            fc_h_v = NoisyLinear(args.dense_of_uav * args.hidden_size, args.hidden_size, std_init=args.noisy_std)
            fc_h_a = NoisyLinear(args.dense_of_uav * args.hidden_size, args.hidden_size, std_init=args.noisy_std)
            fc_z_v = NoisyLinear(args.hidden_size, self.atoms, std_init=args.noisy_std)
            fc_z_a = NoisyLinear(args.hidden_size, action_space * self.atoms, std_init=args.noisy_std)
            self.back_processor.append(((fc_h_v, fc_h_a), (fc_z_v, fc_z_a)))
    ...
    def forward(self, x, log=False):
        ...
        out_q = []
        for layers in self.back_processor:
            v = layers[1][0](F.relu(F.dropout(layers[0][0](x), p=0.2)))  # Value stream
            a = layers[1][1](F.relu(F.dropout(layers[0][1](x), p=0.2)))  # Advantage stream
            # v = layers[1][0](F.relu(layers[0][0](x)))  # Value stream
            # a = layers[1][1](F.relu(layers[0][1](x)))  # Advantage stream
            v, a = v.view(-1, 1, self.atoms), a.view(-1, self.action_space, self.atoms)
            q = v + a - a.mean(1, keepdim=True)  # Combine streams
            if log:  # Use log softmax for numerical stability
            ...
            out_q.append(q)
        return torch.cat(out_q, dim=1)

    def reset_noise(self):
        for name, module in self.named_children():
            if 'fc' in name and name != 'fc':
                module.reset_noise()
        for x, y in self.back_processor:
            for m, n in zip(x, y):
                m.reset_noise()
                n.reset_noise()
